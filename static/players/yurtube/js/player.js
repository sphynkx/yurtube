(function () {
  ...
  function parseJSONAttr(el,name,fallback){ var s=el.getAttribute(name); if(!s)return fallback; try{return JSON.parse(s);}catch(_){return fallback;} }
  function copyText(s){ if(navigator.clipboard&&navigator.clipboard.writeText){ navigator.clipboard.writeText(s).catch(function(){}); } else { var ta=document.createElement("textarea"); ta.value=s; ta.style.position="fixed"; ta.style.left="-9999px"; document.body.appendChild(ta); ta.select(); try{document.execCommand("copy");}catch(e){} document.body.removeChild(ta);} }
  function cssPxToNum(s){ var v=parseFloat(String(s||"").trim()); return isFinite(v)?v:0; }
  function detectPlayerName(){ try{ var u=new URL(import.meta.url); var m=u.pathname.match(/\/static\/players\/([^\/]+)\//); if(m)return m[1]; }catch(e){} try{ var s=(document.currentScript&&document.currentScript.src)||""; var m2=s.match(/\/static\/players\/([^\/]+)\//); if(m2)return m2[1]; }catch(e2){} try{ var host=document.querySelector('.player-host[data-player]'); if(host){ var pn=String(host.getAttribute('data-player')||'').trim(); if(pn)return pn; } }catch(e3){} return "yurtube"; }
  ...
  function installFallbackGuards(video,sourceEl,onDebug){
    ...
    function applyFallback(reason){ if(applied)return; applied=true; onDebug&&onDebug("fallback: applying",reason); try{ if(sourceEl) sourceEl.setAttribute("src",FALLBACK_SRC); else video.src=FALLBACK_SRC; video.load(); }catch(e){} }
    function clearWatchdog(){ if(watchdog){ clearTimeout(watchdog); watchdog=null; } }
    video.addEventListener("loadstart",function(){ clearWatchdog(); watchdog=setTimeout(function(){ if(!applied && video.readyState<1) applyFallback("watchdog-timeout"); },4000); });
    ["loadeddata","canplay","canplaythrough","play","playing"].forEach(function(ev){ video.addEventListener(ev, clearWatchdog); });
    video.addEventListener("error", function(){ if(!applied) applyFallback("error-event"); });
    setTimeout(function(){ var src=sourceEl?(sourceEl.getAttribute("src")||""):(video.currentSrc||video.src||""); if(!applied&&!src) applyFallback("empty-src"); },0);
  }
  ...
  function mountOne(host, tpl, BASE){
    ...
    if(captionVtt){
      ...
      }catch(e){ d("caption append failed",e); }
    }

    try{ video.load(); d("video.load()", {src:videoSrc}); }catch(e){ d("video.load() error", e); }
    ...
    [["--icon-play","play.svg"],
     ...
     ["--icon-autoplay-off","autoplay-off.svg"]].forEach(function(p){
    ...
    Object.assign(overlay.style,{
      ...
      zIndex:"21",pointerEvents:"auto",userSelect:"none",touchAction:"none"
    });
    var textBox=document.createElement("div");
    textBox.className="yrp-captions-text";
    Object.assign(textBox.style,{
      display:"inline-block",background:"rgba(0,0,0,0.55)",color:"#fff",
      fontSize:"16px",lineHeight:"1.35",padding:"6px 10px",borderRadius:"6px",
      maxWidth:"80%",minWidth:"220px",boxShadow:"0 2px 6px rgba(0,0,0,0.35)",
      whiteSpace:"pre-wrap",wordBreak:"break-word"
    });
    overlay.appendChild(textBox);
    if(vw){ vw.style.position="relative"; vw.appendChild(overlay); }

    var dragState={active:false,startX:0,startY:0,startLeftPct:50,startTopPct:80};
    var userMoved=false;
    function getVideoWrapRect(){ return vw? vw.getBoundingClientRect():{width:0,height:0,left:0,top:0}; }
    function updateTextBoxMaxWidthByCenter(cx){
      var r=getVideoWrapRect();
      var leftAvail=cx, rightAvail=r.width-cx;
    ...
    function startDrag(e){
      ...
      var r=getVideoWrapRect(), ov=overlay.getBoundingClientRect();
    ...
    function moveDrag(e){
      ...
      var t=e.touches?e.touches[0]:e;
      var dx=t.clientX-dragState.startX, dy=t.clientY-dragState.startY;
      var r=getVideoWrapRect();
      var newLeftPx=(dragState.startLeftPct/100)*r.width + dx;
      var newTopPx =(dragState.startTopPct /100)*r.height+ dy;
      var ov=overlay.getBoundingClientRect(), halfW=ov.width/2, halfH=ov.height/2;
    ...
    function endDrag(e){ if(dragState.active){ dragState.active=false; e.preventDefault(); e.stopPropagation(); } }
  ...
  function wire(root,startAt,DEBUG,hooks){
    ...
    function subtitleTracks(){
      try{ return video.textTracks? Array.prototype.filter.call(video.textTracks,function(tr){ return tr.kind==="subtitles"||tr.kind==="captions"; }):[]; }catch(_){ return []; }
    }
    ...
    function logTracks(prefix){
      var subs=subtitleTracks();
      var info=subs.map(function(tr,i){ return {i:i,mode:tr.mode,label:tr.label,srclang:tr.language||tr.srclang,cues:tr.cues?tr.cues.length:0,kind:tr.kind}; });
      d(prefix, info);
    }
    ...
    var mirrorLang=""; var mirrorLabel=""; // for reuse
    ...
    function parseTimestamp(ts){ var m=String(ts||"").match(/^(\d{2}):(\d{2}):(\d{2}\.\d{3})$/); if(!m)return 0; return parseInt(m[1],10)*3600+parseInt(m[2],10)*60+parseFloat(m[3]); }
    function buildAbsoluteSpriteUrl(rel){ if(!rel)return ""; if(/^https?:\/\//i.test(rel)||rel.startsWith("/")) return rel; try{ var u=new URL(spritesVttUrl,window.location.origin); var base=u.pathname.replace(/\/sprites\.vtt$/,""); return base+"/"+rel.replace(/^\/+/,""); }catch(e){ return rel; } }
    function ensureSpritePop(){ if(spritePop) return spritePop; if(!progress) return null;
      spritePop=document.createElement("div");
      Object.assign(spritePop.style,{position:"absolute",display:"none",bottom:"calc(100% + 30px)",left:"0",width:"160px",height:"90px",border:"1px solid #333",background:"#000",overflow:"hidden",zIndex:"5"});
        ...
        for(var i=0;i<lines.length;i++){
          ...
          if(line.indexOf("-->")>=0){
            ...
            if(hashIdx>0){
              ...
              if(xywh.length===4){ x=parseInt(xywh[0],10); y=parseInt(xywh[1],10); w=parseInt(xywh[2],10); h=parseInt(xywh[3],10); }
            }
    ...
    function showSpritePreviewAtClientX(clientX){
      ...
      var cue=null;
      for(var i=0;i<spriteCues.length;i++){ var c=spriteCues[i]; if(t>=c.start && t<c.end){ cue=c; break; } }
      var pop=ensureSpritePop(); if(!pop) return;
      ...
      img.src=cue.spriteUrl; pop.appendChild(img);
      pop.style.display="block";
      var leftPx=clamp(x-cue.w/2,0,rect.width-cue.w);
      pop.style.left=leftPx+"px"; pop.style.width=cue.w+"px"; pop.style.height=cue.h+"px";
    }
    ...
    function refreshAutoplayBtn(){
      ...
      Object.assign(btnAutoplay.style,{
        backgroundColor:"#6cc9fa",webkitMaskImage:iconVar,maskImage:iconVar,
        webkitMaskRepeat:"no-repeat",maskRepeat:"no-repeat",
        webkitMaskPosition:"center",maskPosition:"center",
        webkitMaskSize:"20px 20px",maskSize:"20px 20px"
      });
    }
    ...
    function scheduleAutoHide(ms){ if(hideTimer) clearTimeout(hideTimer); hideTimer=setTimeout(function(){ root.classList.add("autohide"); hooks&&hooks.autoAdjust&&hooks.autoAdjust(); }, Math.max(0,ms||1200)); }
    function showControls(){ root.classList.remove("autohide"); hooks&&hooks.autoAdjust&&hooks.autoAdjust(); scheduleAutoHide(2000); }
    ...
    function updateProgress(){
      ...
      var bf=d>0?clamp(b/d,0,1):0; if(buf) buf.style.width=(bf*100).toFixed(3)+"%";
    }
    ...
    function refreshVolIcon(){
      ...
      if(btnVol){ btnVol.textContent=label; btnVol.classList.toggle("icon-mute",label==="Mute"); btnVol.classList.toggle("icon-vol",label!=="Mute"); }
    }
    ...
    function hideMenus(){
      if(menu){ menu.hidden=true; btnSettings&&btnSettings.setAttribute("aria-expanded","false"); }
    ...
    (function volumeResume(){
      ...
      video.addEventListener("loadedmetadata",function once(){ video.removeEventListener("loadedmetadata",once);
    ...
    (function theaterInit(){
      ...
      btnTheater.addEventListener("click", function(){
        ...
        if(inTheater){ root.style.maxWidth=""; root.style.minWidth=""; root.style.width=""; } else { adjustWidthByAspect(); }
        showControls();
      });
    })();
    ...
    (function autoplayInit(){
      ...
      var fired=false; function fireOnce(){ if(fired)return; fired=true; sequence(); }
    ...
    video.addEventListener("loadedmetadata", function(){
      if(startAt>0){ try{ video.currentTime=Math.min(startAt, Math.floor(video.duration||startAt)); }catch(e){} }
      setTimeout(adjustWidthByAspect,0);
      updateTimes(); updateProgress(); refreshVolIcon(); refreshAutoplayBtn(); refreshPlayBtn();
    ...
    if(volSlider){
      volSlider.addEventListener("input", function(){
        ...
        v=clamp(v,0,1); video.volume=v; if(v>0) video.muted=false; refreshVolIcon();
      });
      volSlider.addEventListener("wheel", function(e){
        ...
        var nv=clamp(v+(e.deltaY<0?step:-step),0,1);
        video.volume=nv; if(nv>0) video.muted=false;
        volSlider.value=String(nv); refreshVolIcon(); showControls();
      }, {passive:false});
    }
    ...
    btnAutoplay && btnAutoplay.addEventListener("click", function(e){
      e.preventDefault(); e.stopPropagation();
      var next=!load("autoplay",false); save("autoplay",next); autoplayOn=next; refreshAutoplayBtn();
    });
    ...
    root.addEventListener("contextmenu", function(e){
      ...
      ctx.style.left=(e.clientX-rw.left)+"px"; ctx.style.top=(e.clientY-rw.top)+"px"; ctx.hidden=false;
      ctx.onclick=function(ev){
        var act=ev.target&&ev.target.getAttribute("data-action");
        var at=Math.floor(video.currentTime||0);
        var vid=root.getAttribute("data-video-id")||"";
        if(act==="pip") toggleMini();
        else if(act==="copy-url"){ var u=new URL(window.location.href); u.searchParams.delete("t"); copyText(u.toString()); }
        else if(act==="copy-url-time"){ var u2=new URL(window.location.href); u2.searchParams.set("t", String(at)); copyText(u2.toString()); }
        else if(act==="copy-embed"){ var src=(window.location.origin||"")+"/embed?v="+encodeURIComponent(vid||""); var iframe="<iframe width=\"560\" height=\"315\" src=\""+src+"\" frameborder=\"0\" allow=\"autoplay; encrypted-media; clipboard-write\" allowfullscreen></iframe>"; copyText(iframe); }
        ctx.hidden=true;
      };
    ...
    function handleHotkey(e){
      ...
      if(code==="KeyF"||key==="f"){ if(document.fullscreenElement) document.exitFullscreen().catch(function(){}); else root.requestFullscreen&&root.requestFullscreen().catch(function(){}); e.preventDefault(); return; }
