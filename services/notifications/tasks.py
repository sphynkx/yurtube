...
# Redis key prefixes
LIKE_PREFIX = "notif:likes:agg"               # comment likes
VIDEO_LIKE_PREFIX = "notif:video_likes:agg"   # video likes (reactions)
...
def handle_event(body: Dict[str, Any]):
    """
    Entry point for immediate events published via services.notifications.events_pub.publish
    """
...
async def _handle_comment_created(payload: Dict[str, Any]):
    """
    Notify:
      - video author about a new comment on their video
...
async def _handle_comment_reply(payload: Dict[str, Any]):
    # Same as created, it handles reply case too
    await _handle_comment_created(payload)


async def _handle_comment_voted(payload: Dict[str, Any]):
    """
    Batch likes for comments into Redis sets; flushed by flush_like_batches.
    Only likes (vote == 1) are aggregated.
    """
    ...
    try:
        ...
        try:
            for key in to_process:
                ...
                parts = key.split(":")
                # key = notif:likes:agg:{comment_id}:{window} -> id = parts[-2]
...
async def _handle_video_reacted(payload: Dict[str, Any]):
    """
    Aggregate video likes (not dislikes) into Redis sets; flushed by flush_video_like_batches.
    Payload expected:
      { video_id, actor_uid, video_author_uid, title, reaction: 'like' }
    """
    ...
    try:
        ...
        try:
            for key in to_process:
                ...
                parts = key.split(":")
                # key = notif:video_likes:agg:{video_id}:{window} -> id = parts[-2]
                ...
                # Check video directly
                vrow = await conn.fetchrow("SELECT video_id, title FROM videos WHERE video_id = $1", video_id_from_key)
                if not vrow:
...
async def _handle_video_published(payload: Dict[str, Any]):
    """
    Notify subscribers about newly published (public/ready) video.
    """
    video_id = payload.get("video_id")
