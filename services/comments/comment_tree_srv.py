...
async def fetch_texts_for_comments(video_id: str, root_doc: Dict[str, Any], show_hidden: bool) -> Dict[str, str]:
    # Collect set of local_id - by chunk_id
    ...
    out: Dict[str, str] = {}
    for chunk_id, lids in by_chunk.items():
        chunk = await chunk_coll().find_one({"_id": chunk_id})
        if not chunk:
            continue
...
def build_tree_payload(root_doc: Dict[str, Any], show_hidden: bool) -> Dict[str, Any]:
    # Return struct suitable for UI: roots list, map by cid
    ...
    # Filter by visible (if not a moderator)
    if not show_hidden:
